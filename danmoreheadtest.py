...
def fibonacci_retracement(close_prices):
    max_price = max(close_prices)
    min_price = min(close_prices)
    diff = max_price - min_price
    levels = [0.0, 0.236, 0.382, 0.5, 0.618, 0.786, 1.0]
    return [min_price + diff * level for level in levels]


def is_double_top_or_bottom(close_prices):
    # Define a threshold to determine similarity in peak/trough values
    threshold = 0.02
    # Find maxima and minima indices
    max_idx = [
        i
        for i in range(1, len(close_prices) - 1)
        if close_prices[i - 1] < close_prices[i] > close_prices[i + 1]
    ]

    if len(max_idx) < 2:
        return False

    # Check if the last two peaks are approximately the same value (i.e. within the threshold)
    if abs(close_prices[max_idx[-1]] - close_prices[max_idx[-2]]) < threshold:
        return True
    return False


def strategy(candle, close_prices, sma_period):
    ...
    # Triangle and Flag patterns can be approximated by checking if price deviates from the SMA
    deviation_from_sma = abs(close_price - average) / average
    if deviation_from_sma > 0.01:
        return False  # Price deviates too much, doesn't look like a triangle or flag

    # Check for Double Top or Double Bottom pattern
    if is_double_top_or_bottom(close_prices):
        return False  # Avoid buying at potential reversal points

    # Check Fibonacci Retracement Levels
    fib_levels = fibonacci_retracement(close_prices)
    if any([level * 0.98 < close_price < level * 1.02 for level in fib_levels]):
        return False  # Avoid buying at potential reversal points
...
def main(starting_balance, order_balance_factor, take_profit_factor, stop_loss_factor):
    ...
    for i in range(1, len(klines)):
        ...
        # can open order?
        if open_order is None and strategy(prev_candle, close_prices, sma_period):
